...
export class SchedulerOrchestrator implements OnApplicationBootstrap, OnApplicationShutdown {
    ...
    constructor(private readonly registry: SchedulerRegistry, private readonly wrapper: ScheduleWrapper) {}
    ...
    public mountIntervals() {
        this.registry.getIntervalJobs().forEach((job, name) => {
            ...
            let methodRef = this.wrapper.retryable(target as any, options.retries, options.retry);
    ...
    public mountTimeouts() {
        this.registry.getTimeoutJobs().forEach((job, name) => {
            ...
            const methodRef = this.wrapper.lockable(
                name,
                locker,
                this.wrapper.retryable(target as any, options.retries, options.retry)
            );
    ...
    addTimeout(methodRef: Function, timeout: number, name: string, locker: Locker, options?: TimeoutOptions) {
    ...
    addInterval(methodRef: Function, interval: number, name: string, locker: Locker, options?: IntervalOptions) {
